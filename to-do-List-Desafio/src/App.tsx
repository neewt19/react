...
function App() {
  ...
  const [tasks, setTasks] = useState([{ id: 0, content: ' Estudar React ', checked: false }])
  ...
  const toggleTask = (taskId: number, checked: boolean) => {
    const updatedTasks = tasks.map(task => {
      const tasksreturn = task.id === taskId ? { ...task, checked: !task.checked } : task;
      return tasksreturn;
    });
    ...
    if (tasks.length === 0) {
      return (
        <div className={styles.noTasks}>
          ...
          <span className={styles.spanBold}>Você ainda não tem tarefas cadastradas</span>
      ...
      <div className={styles.fundo}>
        ...
        <div className={styles.container}>
          ...
          <div className={styles.imputArea}>
            <input value={newTask} type="text" onChange={handleChange} placeholder='Adicione uma nova tarefa' />
          ...
          <div className={styles.tasksCount}>
            ...
            <div>
              ...
              <span className={styles.taskCountNumbers}>{count} de {tasks.length}</span>
            </div>
          </div>
          {renderItem()}
          {tasks.map(tasks => {
